      ...
      <div className=' grid grid-cols-4 gap-4 lg:gap-[30px]'>
        {bundleData.map((item,idx)=>{
          ...
          onClick={()=>{
           ...
           text-center'>
            {/* image  */}
            <div className=' mb-2 lg:mb-8 hover:scale-105
             transition-all duration-300'>
              <img className=' w-full' src={image.type} alt="" />
            </div>
            <h3 className=' lg:text-2xl capitalize
             font-semibold text-blue lg:mb-2'>{name}</h3>
            {/* category  */}
            <div className={`${index ===idx ? "border-b-4 border-orange transition-allafter:lg:w-5 after:lg:h-5 after:bg-triangle after:lg:bg-no-repeat after:lg:block after:lg:left-[130px] after:lg:absolute after:lg:-bottom-6 relative":" border-b-4 border-transparent"}
             pb-4 mb-12`}>
              <div className=' hidden lg:block font-medium'>{dogCategory}</div>
            </div>
          </div>
        })}
      </div>
